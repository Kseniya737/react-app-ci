...
export const stringIsOfOption = <T extends string>(stringLevel: string, options: readonly string[]): T =>{
    if (stringLevelIsT(stringLevel, options)){
        return stringLevel as T
    }
    throw Error(`String ${stringLevel} needs to be one of the ${options}`)
}